...
def cmp(f, s):
    nf = np.load(f"data/{s}.npz")['data']
    return f.values.numpy(f.values.shape.names) - nf
...
class TestRun:
    ...
    def pt_low_ord(self, v, p, dt_=None):
        if dt_ is None:
            dt_ = self.dt
        v, delta_p = \
            fluid.make_incompressible(v, solve=math.Solve('biCG-stab(2)', 1e-10, 1e-10)) # "scipy-GMres" wirft fehler "biCG-stab(2)" geht
        p += delta_p / dt_
        return v, p
