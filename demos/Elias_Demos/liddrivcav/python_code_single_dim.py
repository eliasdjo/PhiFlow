...
class TestRun:
    ...
    def run(self, jit_compile=True, t_num=0, freq=100):
        ...
        if self.order == 'phi':
            ...
            DOMAIN = dict(bounds=Box['x', 0:self.xynum], x=self.xynum,
            ...
            DOMAIN2 = dict(bounds=Box['x', 0:self.xynum], x=self.xynum,
        ...
        velocity = self.gridtype(tensor([0]), **DOMAIN)
        ...
        for i in range(5):
            velocity, pressure = fluid.make_incompressible2(velocity, order=2, solve=math.Solve('scipy-direct', 1e-6, 1e-6))
